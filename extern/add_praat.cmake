...
# add source files to praat target
function(AddFilesToPraat)
	list(POP_FRONT ARGV SUBDIR)
	list(TRANSFORM ARGV PREPEND "${DIRPATH}/")
	target_sources(praat PRIVATE ${ARGV})
endfunction()

# add all source files to praat target in specified subdirectory
function(AddAllFilesToPraat)
	# first argument=subdir name; rest=list of sources to be excluded
	list(POP_FRONT ARGV SUBDIR)
	...
	# if exclusion files are given
	foreach (exclude ${ARGV})
		string(JOIN "/" exclude ${SUBDIR} ${exclude})
		# message(STATUS "Excluding ${exclude}")
		list(REMOVE_ITEM ALL_SOURCES ${exclude})
	endforeach()

	# screen out source files for other os's
	foreach (src ${ALL_SOURCES})
		if (${src} MATCHES "_unix_")
			if (UNIX)
				list(APPEND SOURCES ${src})
			endif()
		elseif(${src} MATCHES "_linux_")
			if (CMAKE_SYSTEM_NAME EQUAL "Linux")
				list(APPEND SOURCES ${src})
			endif()
		elseif(${src} MATCHES "_mac_")
			if (CMAKE_SYSTEM_NAME EQUAL "Darwin")
				list(APPEND SOURCES ${src})
			endif()
		elseif(src MATCHES "_win_")
			if (WIN32)
				list(APPEND SOURCES ${src})
			endif()
		else()
	...
	# add sources
	target_sources(praat PRIVATE ${SOURCES})
endfunction()

foreach(subdir ${PRAAT_SUBDIRECTORIES})
	#prepend praat base dir name
	string(JOIN "/" DIRPATH "praat" ${subdir})

	# message(STATUS "Scanning ${DIRPATH} for source files and header directories")

	#add all source files in specified praat subdirectories
	if (${subdir} STREQUAL "external/gsl")
		string(JOIN "/" FULLPATH ${CMAKE_CURRENT_SOURCE_DIR} ${DIRPATH})
		AddFilesToPraat(${DIRPATH} 
		...
		AddAllFilesToPraat(${DIRPATH})
	endif()

	# glob all directories with header files in PRAAT_SUBDIRECTORIES
	...
	foreach(hdr ${ALL_HEADERS})
		...
		if (NOT (${dir} IN_LIST PRAAT_INCLUDE_DIRS))
			list(APPEND PRAAT_INCLUDE_DIRS ${dir})
			# message(STATUS "Appended ${dir}")
		endif()
	endforeach(hdr ${ALL_HEADERS})
endforeach(subdir ${PRAAT_SUBDIRECTORIES})
