...
namespace parselmouth
{
	...
	PRAAT_STRUCT_BINDING(Frame, Pitch_Frame)
	{
		...
		def(
			...
			[](Pitch_Frame self, long i) {
				if (i < 0)
					i += self->nCandidates; // Python-style negative indexing
				if (i < 0 || i >= self->nCandidates)
					throw py::index_error("Pitch Frame index out of range");
	...
	PRAAT_CLASS_BINDING(Pitch)
	{
		...
		def(
			...
			[](Pitch self, long i) {
				if (i < 0)
					i += self->nx; // Python-style negative indexing
				if (i < 0 || i >= self->nx)
					throw py::index_error("Pitch index out of range");
		...
		def(
			...
			[](Pitch self, std::tuple<long, long> ij) {
				...
				if (j < 0)
					j += frame.nCandidates; // Python-style negative indexing
				if (j < 0 || j >= frame.nCandidates)
					throw py::index_error("Pitch Frame index out of range");
