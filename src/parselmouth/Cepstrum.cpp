...
PRAAT_CLASS_BINDING(Cepstrum) {
	using signature_cast_placeholder::_;

	def(py::init([](py::array_t<double, 0> values, Positive<double> qmax) {
		    auto ndim = values.ndim();
		    if (ndim > 2) {
			    throw py::value_error("Cannot create Cepstrum from an array with more than 2 dimensions");
		    }
		    if (ndim == 2 && values.shape(0) > 2) {
			    throw py::value_error("Cannot create Cepstrum from 2-dimensional array where the first dimension is greater than 2");
		    }

		    auto n = values.shape(ndim - 1);
		    auto result = Cepstrum_create(qmax, n);
	...
	def(py::init([](py::array_t<std::complex<double>, 0> values, Positive<double> qmax) {
		    auto ndim = values.ndim();
		    if (ndim > 1) {
			    throw py::value_error("Cannot create Cepstrum from a complex array with more than 1 dimension");
		    }
	...
	def("__setitem__",
	    [](Cepstrum self, integer index, std::complex<double> value) {
		    if (index < 0 || index >= self->nx)
			    throw py::index_error("bin index out of range");
