	...
	kGraphicsPostscript_paperSize paperSize, kGraphicsPostscript_orientation rotation, double magnification)
{
	...
	// my d_vprintf = [](FILE *stream, const char *format, fmt::printf_args args) { return fmt::vfprintf(stream, format, args); };  
	my d_vprintf = [](FILE *stream, const char *format, fmt::basic_format_args<fmt::printf_context> args) { return fmt::vfprintf(stream, fmt::string_view(format), args); };
